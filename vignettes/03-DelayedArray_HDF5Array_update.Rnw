...
\begin{frame}[fragile]
  ...
  \begin{block}{}
    ...
    \begin{itemize}
      \item Implemented in C (calls to \Biocpkg{Rhdf5lib})
      \item Operates at the level of the physical chunks
      \item More efficient than blockApply()
    \end{itemize}
  \end{block}
  ...
  \begin{block}{}
    \Rcode{SparseArray} objects:
    \begin{itemize}
      \item In-memory sparse representation of arrays of arbitrary dimensions
      \item Used internally by block processing of sparse \Rcode{DelayedArray}
      ...
      \item Implement fast native operations: arithmetic, \Rcode{Math}
            ...
            Will benefit block processing of sparse \Rcode{DelayedArray}
            objects
    \end{itemize}
  \end{block}
\end{frame}
