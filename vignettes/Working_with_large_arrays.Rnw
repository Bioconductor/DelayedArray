...
\begin{frame}[fragile]
  ...
  All the members of the \Rcode{Arith} (\Rcode{+}, \Rcode{-}, ...),
  \Rcode{Compare} (\Rcode{==}, \Rcode{<}, ...),
  \Rcode{Logic} (\Rcode{\&}, \Rcode{|}),
  \Rcode{Math} (\Rcode{log}, \Rcode{sqrt}), and \Rcode{Math2} groups
  are delayed:
...
{\scriptsize
<<>>=
M6 <- round(M[11:15, ] / M[1:5, ], digits=3)
M6
@
}
      \end{exampleblock}
...
\begin{frame}[fragile]
  ...
  \centerline{\bf Controlling the block size}
