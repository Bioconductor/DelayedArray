...
\begin{frame}[fragile]
  \frametitle{{\bf dgCMatrix} vs {\bf RleMatrix} vs {\bf HDF5Matrix}}
  \begin{exampleblock}{}
<<>>=
...
\begin{frame}[fragile]
  {\bf Controlling where HDF5 datasets are realized}
  \begin{block}{}
  \end{block}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{block}{}
    2. It's wrapped in a SummarizedExperiment. Get the count data as an
       ordinary matrix.
  \end{block}
...
\begin{frame}[fragile]
  ...
  \begin{block}{}
    8. Use \Rcode{DelayedArray:::set\_verbose\_block\_processing} to see
...
\begin{frame}[fragile]
  {\bf Block processing improvements}
  \begin{block}{}
