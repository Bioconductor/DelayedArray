...
The `path` slot should be a single string that contains the absolute
path to the ADS file so the object doesn't break when the user changes
the working directory (e.g. with `setwd()`). Note that storing an open
...
It is highly recommended to provide a "seed constructor" e.g.:
    ...
    ADSArraySeed <- function(path, other args)
    {
        sanity checks
        ...
        new("ADSArraySeed", path=path, other args)
    }
...
`matrixClass` is a generic function defined in the DelayedArray package.
...
The `ADSArray` constructor should just do that:
    ...
    ADSArray <- function(path, other args)
        DelayedArray(ADSArraySeed(path, other args))
...
with something like:
    ...
    ADSArray <- function(path, other args)
    {
        if (is(path, "ADSArraySeed")) {
            ...
            seed <- ADSArraySeed(path, other args)
        }
        DelayedArray(seed)
    }
