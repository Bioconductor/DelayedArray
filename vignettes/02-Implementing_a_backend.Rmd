...
(with `M2 <- t(M)`) will demote the object to a DelayedArray or DelayedMatrix
instance, causing `max(A)` and `max(M2)` to use the far less efficient
...
to make the `max` method (and other block-processed methods) for DelayedArray
objects try to take advantage of the backend-specific method whenever it can.
