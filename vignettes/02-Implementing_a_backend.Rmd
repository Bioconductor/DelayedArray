...
The `file` slot should be a single string that contains the absolute path
to the ADS file so the object doesn't break when the user changes the
working directory (e.g. with `setwd()`). Note that storing an open
connection to the file should be avoided because connections don't work
properly in the context of a fork (e.g. when processing the seed object
in parallel) and tend to break when serializing the object.
...
Note that the `extract_array` method needs to support empty
list elements e.g. `extract_array(x, list(NULL, integer(0)))`
must return an M x 0 matrix of type `complex` and
`extract_array(x, list(integer0, integer(0)))`
...
on it to work. More precisely, once the `extract_array` method
...
They should extend DelayedArray without trying to add slots to it.
So just:
...
## Coercion from ADSArray to ADSMatrix
...
with something like:
    ...
    ADSArray <- function(file, other args)
    {
        if (is(file, "ADSArraySeed")) {
            ...
            seed <- ADSArraySeed(file, other args)
        }
        DelayedArray(seed)
    }
